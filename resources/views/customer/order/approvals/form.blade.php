        ...
        <div class="card-body">
            @if($order)
                <div class="row mb-4">
                    <div class="col-4">
                    <span>
                        <b>{{__('Ordered By :')}}</b> {{ $order->contact->name ?? 'N/A' }}
                    </span>
                    </div>

                    <div class="col-4">
                    <span>
                        <b>{{__('Date Ordered :')}}</b> {{ carbon_date($order->created_at) }}
                    </span>
                    </div>
                    ...
                    <div class="col-4">
                    <span>
                        <b>{{__('Order Reference :')}}</b> {{ $order->customer_order_number }}
                    </span>
                    </div>

                    <div class="col-4">
                    <span>
                        <b>{{__('PO Number :')}} </b> {{ $order->customer_order_number }}
                    </span>
                    </div>

                    <div class="col-4">
                    <span>
                        <b>{{__('Shipping Method :')}} </b> {{ $order->shipping_method }}
                    </span>
                    </div>
            ...
            <div class="form-group">
                <textarea class="form-control" rows="4" id="note"
                          @if ($approval_data->passed != 0) readonly @endif> {{$approval_data->notes}}</textarea>
            </div>

            <input type="hidden" id="approval_id" value="{{ $approval_data->id }}">

            @if ($approval_data->status == 'pending' && $approval_data->approver_id == customer(true)->id && customer(true)->can('order-approval.approve'))
                <div class="form-group">
                    <button class="btn btn-danger" onclick="updateAction('rejected')" type="submit">Reject</button>
                    <button class="btn btn-success" onclick="updateAction('approved')" type="submit">Approve</button>
                </div>
            @endif
        </div>
    </div>
...
<script>
    function updateAction(status) {
        ...
        if (status == 'rejected' && note.length == 0) {
            ShowNotification('error', 'Note', "Please provide order reject note.");
            return 0;
        }
        var redirectTo = "{{ url('/order-approvals') }}";
        $.ajax({
            url: "{{ route('frontend.order.approval') }}",
            type: 'POST',
            data: reqData,
