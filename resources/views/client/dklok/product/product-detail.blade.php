        ...
        <div class="row">
            ...
            <div class="col-md-7">
                ...
                @if($isMasterProduct($product))
                    @include('widget::client.dklok.product.inc.sku-filters')
                @else
                    @include('widget::client.dklok.product.inc.price-qty')
                @endif
            </div>
                ...
                <div class="tab-content p-3 bg-white" id="productTabsContent">
                    @foreach($tabs as $tab)
                        <div class="tab-pane fade {{ $tab['active'] ? 'show active' : '' }} {{ $tab['id'] == 'skus' ? 'products-table' : '' }}" id="{{ $tab['id'] }}" role="tabpanel" aria-labelledby="{{ $tab['id'] }}-tab">
                            ...
                            @elseif($tab['id'] == 'view')
                                @include('widget::client.dklok.product.inc.traceparts-cad-view')
                            @endif
                        </div>
                    @endforeach
                </div>
