<div class="products">
    ...
    <div class="product-card product-list">
        ...
        <div class="product-info">
            <p class="mb-0">
                @if(!empty($product->manufacturer))
                    <a
                        ...
                        href="{{ frontendShopURL('-Manufacturer:'.$product->manufacturer) }}"
                    >
            ...
            <small>MFR Part #: {{$product->MPN ?? ''}} <span class="ml-3">RHS Part #: {{$product->Product_Code ?? ''}} </span></small>
            @if(! empty($product->ERP[0]))
                <div class="product-price mt-2">
                    <b>Your Price: </b><span class="text-danger fw-600">$ {{ $product->ERP[0]->Price ? number_format($product->ERP[0]->Price, 2, '.', '') : '' }}</span>
                    <span class="ml-3">List Price: </span> <span class="">$ {{  $product->ERP[0]->ListPrice ?: $product->ERP[0]->ListPrice }}</span>
                </div>
            @else
                <p class="text-danger">
            ...
            @endif
            <div class="product-buttons d-flex justify-content-between flex-column flex-md-row gap-3">
                <div class="d-flex align-items-center gap-3">
                    @if($product->wareHouse->count() > 0)
                        <b class="text-nowrap">In Stock</b>
                        <div class="row pl-2">
                            @foreach($product->wareHouse as $warehouse)
                                <div class="col-6 px-1 d-flex flex-column w-100 gap-2">
                ...
                <div class="d-flex flex-column gap-3">
                    <div class="product-count align-items-center d-flex justify-content-between">
                        <span
                            class="text-dark border rounded d-flex align-items-center justify-content-center fw-600" onclick="productQuentity({{$key}},'minus')"><i
                                class="icon-minus fw-700"></i></span>
                        <div
                            class="mx-2 px-2 h-100 fw-600 d-flex align-items-center justify-content-center">

                            @include('widget::client.rhsparts.product.inc.partial',['product'=>$product,'key'=>$key])
                            </div>
                        <span
                            class="text-dark border rounded d-flex align-items-center justify-content-center fw-600" onclick="productQuentity({{$key}},'plus')"><i
                    ...
                    <button class="btn btn-primary btn-sm m-0"
                        ...
                        data-toast-icon="icon-circle-check">Add to Cart
                    </button>
